...
const getObj = <S extends Obj | ObjEffect>(data: S) => {
  if (data.constructor.name === z.ZodEffects.name) return (data as z.ZodEffects<Obj>)._def.schema
  return data as Obj
}
...
export const useForm = <S extends Obj | ObjEffect>(schema: S, initialData?: z.TypeOf<S>) => {
  ...
  const { form, valid, ...others } = useZodactiveForm<S, Ref<unknown>>(
    {
      createReactive,
      getReactive,
      setReactive
  ...
  return { form: newForm, valid: newValid, ...others }
}
